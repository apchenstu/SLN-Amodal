...
class AmodalEval:
    def __init__(self, amodalGt=None, amodalDt=None,limit=-1):
        ...
        if not amodalGt is None:
            self.params.imgIds = amodalGt.getImgIds()[:limit]
            self.params.catIds = [1]#i for i in range(61)
    ...
    def _prepare(self):
                ...
                for region in obj['regions']:
                    if type(region['segmentation']) == list:
                        ...
                        if 'area' not in region:
                            ...
                            region['segmentation'] = \
                            mask.frPyObjects([region['segmentation']], t['height'], t['width'])[0]
                        elif type(region['segmentation']) == dict and \
                                type(type(region['segmentation']['counts']) == str):
        ...
        gts = self.amodalGt.dataset.loadAnns(self.amodalGt.dataset.getAnnIds(imgIds=p.imgIds))

        # def _toMask2(objs, coco):
        #     # modify segmentation by reference
        #     for region in objs:
        #         t = coco.imgs[region['image_id']]
        #         #for region in obj:
        #         # format is xyxy, convert to RLE
        #         #region['segmentation'] = mask.frPyObjects([region['segmentation']], t['height'], t['width'])
        #         print(len(region['segmentation']))
        #         if len(region['segmentation']) == 1:
        #             region['segmentation'] = region['segmentation'][0]
        #         else:
        #             # an object can have multiple polygon regions
        #             # merge them into one RLE mask
        #             region['segmentation'] = mask.merge(region['segmentation'])
        #         if 'area' not in region:
        #             region['area'] = mask.area([region['segmentation']])[0]
        #
        #         elif type(region['segmentation']) == dict and type(
        ...
        for gt in gts:
           ...
           #  for g in gt['regions']:
           #      self._gts[gt['image_id'], g['category_id']].append(gt)
        for dt in dts:
    ...
    def evaluate(self):
        ...
        # import pickle
        # with open('dts.out', 'rb') as fp:
        #     self._dts = pickle.load(fp, encoding='bytes')
        # with open('dts.out', 'rb') as fp:
        #     self._gts = pickle.load(fp, encoding='bytes')
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet, oRng):
        ...
        gtind = [ind for (ind, g) in sorted(enumerate(gt), key=lambda x: x[1]['_ignore'])]
        ...
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    dtm[tind, dind] = gt[m]['amodal_region']['order']#gt[m]['order']#
                    gtm[tind, m] = d['id']
        ...
        return {
            ...
            'gtIds': [g['amodal_region']['order'] for g in gt],#[g['order'] for g in gt],#
            'dtMatches': dtm,
            'gtMatches': gtm,
...
class Params:
    ...
    def __init__(self):
        ...
        self.areaRng = [[0 ** 2, 1e5 ** 2]]     # all [0,1], none: [0, 0.00001],partial: [0.00001, 0.25] heavy: [0.25, 1]
        self.useSegm = 1
        self.useAmodalGT = 1
        self.useAmodalDT = 1
        self.onlyThings = 0  # 1: things only; 0: both; 2 stuff only
        self.useCats = 1
